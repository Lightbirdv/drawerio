...
const execute = async () => {
	try {
		...
		let firstuser = await client.query(`INSERT INTO users (email,password,isAdmin) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`, [
			"seedtestadminuser",
			await bcrypt.hash("seedtestadminpassword", 10),
			"true",
		]);
		console.log("Testadminuser created successfully");
		let seconduser = await client.query(`INSERT INTO users (email,password) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [
			"seedtestuser",
