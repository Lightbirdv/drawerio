...
const execute = async () => {
	try {
		...
		let firstuser = await client.query(`INSERT INTO users (email,password,isAdmin,enabled) VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`, [
			"seedtestadminuser",
			await bcrypt.hash("seedtestadminpassword", 10),
			"true",
			true,
		]);
		console.log("Testadminuser created successfully");
		let seconduser = await client.query(`INSERT INTO users (email,password,enabled) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`, [
			"seedtestuser",
			await bcrypt.hash("seedtestpassword", 10),
			true,
		]);
		console.log("Testuser created successfully");
		let firstdrawer = await client.query("INSERT INTO drawer (drawerTitle, creationDate, users_id) VALUES ($1,$2,$3)   RETURNING *", [
			"testdrawer for adminuser",
			new Date(),
			1,
		]);
		console.log("First drawer created successfully");
		let seconddrawer = await client.query("INSERT INTO drawer (drawerTitle, creationDate, users_id) VALUES ($1,$2,$3)   RETURNING *", [
