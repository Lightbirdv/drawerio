...
async function getEntries(req: express.Request, res: express.Response) {
  const entries = await pool.query(
    "SELECT * FROM drawerentries ORDER BY drawerentry_id ASC"
  );
  return entries.rows;
}
...
async function getEntriesByDrawer(req: express.Request, res: express.Response) {
  const entries = await pool.query(
    "SELECT * FROM drawerentries where drawer_id=$1 ORDER BY drawerentry_id ASC",
    [req.params.drawerid]
  );
  return entries.rows;
}
...
async function deleteEntry(req: any, res: express.Response) {
  ...
  const entry = pool.query(
    "DELETE FROM drawerentries WHERE drawerentry_id=$1",
    [req.params.id]
  );
  return entry;
}
