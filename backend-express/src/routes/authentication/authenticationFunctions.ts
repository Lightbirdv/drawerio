...
import { User, UncodedToken } from "./authentication";
...
async function authenticateToken(req: any, res: express.Response, next: express.NextFunction) {
	...
	jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, async (err: any, uncodedToken: UncodedToken) => {
		if (err) return res.sendStatus(403);
		let result = await userFunctions.getUserByEmail(uncodedToken.user, res, next);
		if (!result.rows.length) {
...
async function isAdmin(req: any, res: express.Response, next: express.NextFunction) {
	const authHeader = req.headers["authorization"];
	const token = authHeader && authHeader.split(" ")[1];
	jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, async (err: any, uncodedToken: any) => {
		if (err) return res.sendStatus(403);
		let result = await userFunctions.getUserByEmail(uncodedToken.user, res, next);
		if (!result.rows.length) {
			return next(new HttpException(404, "No user found"));
