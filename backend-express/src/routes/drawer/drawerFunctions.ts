...
async function getDrawers() {
  const drawers = await pool.query(
    "SELECT * FROM drawer ORDER BY drawer_id ASC"
  );
  return drawers.rows;
}
...
async function getDrawersByUser(req: any, res: express.Response) {
  const drawers = await pool.query(
    "SELECT * FROM drawer where users_id=$1 ORDER BY drawer_id ASC",
    [req.user.users_id]
  );
  return drawers.rows;
}

async function getSingleDrawer(
  req: express.Request,
...
async function updateDrawer(
  ...
  const newDrawer = pool.query(
    "UPDATE drawer SET drawer_id=$1, drawerTitle=$2, creationDate=$3, users_id=$4  WHERE drawer_id=$5",
    [
...
async function addDrawer(req: any, res: express.Response) {
  ...
  let newDrawer = pool.query(
    "INSERT INTO drawer (drawerTitle,creationDate, users_id) VALUES ($1,$2,$3)   RETURNING *",
